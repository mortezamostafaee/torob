			...
			if ( !$used_acf ) {
				...
				if ( $type == 'post' ) {
					...
					if ( count($args['post_type']) > 0 && !in_array('any', $args['post_type']) ) {
						$post_type_query = "AND p.post_type IN ('".implode("','", self::escape( $args['post_type'] ))."')";
					}
					if ( count($args['post_status']) > 0 && !in_array('any', $args['post_status']) ) {
						$post_status_query = "AND p.post_status IN ('".implode("','", self::escape($args['post_status']))."')";
					}
					if ( count($args['exclude']) > 0 ) {
						$exclude_query = "AND pm.meta_value NOT IN ('".implode("','", self::escape($args['exclude']))."')";
					}
