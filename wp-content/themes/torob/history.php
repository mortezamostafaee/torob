...
$torob_user_history = $wpdb->prefix . 'user_history';
$torob_main_product = $wpdb->prefix . "torob_main_product";
$torob_torob_wishlist = $wpdb->prefix . 'torob_wishlist';
if (isset($_POST['remove-history'])) {
    $id = $user_id;
    $table = $torob_user_history;
    $wpdb->delete($table, array('user_id' => $id));
}
$result1 = $wpdb->get_results("SELECT * FROM $torob_user_history JOIN $torob_main_product on $torob_user_history.product_id = $torob_main_product.product_id where $torob_user_history.user_id = $user_id");
...
<?php if (is_user_logged_in()) : ?>
    <div class="d-flex gap-5">
        <aside class="user-side">
            <a href="<?php echo get_home_url().'/analytics' ?>"> <h6><img src="<?php echo get_template_directory_uri() . '/images/alert1.svg' ?>">تغییرات قیمت</h6></a>
            <a href="<?php echo get_home_url().'/favorites' ?>"> <h6><img src="<?php echo get_template_directory_uri() . '/images/hart.svg' ?>">محبوب ها</h6></a>
            <a href="<?php echo get_home_url().'/history' ?>"></a> <h6 class="user-side-a"><img src="<?php echo get_template_directory_uri() . '/images/time1.svg' ?>">مشاهدات اخیر</h6></a>
            <hr>
            <a href="#">
        ...
        <?php if (!$result1) : ?>
            <main class="main-user-panel">
                <span><img src="<?php echo get_template_directory_uri() . '/images/empty_history.png' ?>"></span>
        ...
        <?php else : ?>
            <main class="main-user-panel2">
                ...
                <h1 style="color: rgb(51, 51, 51); font-weight: bold; font-size: 24px; line-height: 40px; display: inline-block; margin: 0px;">مشاهدات اخیر</h1>

                    <form action="" method="post">
                    <button name="remove-history" type="submit" style="border: none;background: none;color: #d73948; font-size: small;">حذف مشاهدات اخیر</button>
                    </form>
                </div>
                <div class="d-flex gap-3">
                    <?php foreach ($result1 as $item) : ?>
                        ...
                        <div class="product-torob">
                            <a href="<?php echo get_home_url() . '/single-product/?id=' . $item->product_id . '' ?>"> <img
                                        src="<?php echo $item->url_image ?>" style="width: 135px;height: 145px;"></a>
                            ...
                            <div class="d-flex justify-content-around icon-product">
                                <div>  <form action="" method="post" name="wishlist-torob" style="margin-bottom: 0;"
